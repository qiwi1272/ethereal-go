...
func main() {
	...
	client, err := restClient.NewRestClient(ctx, os.Getenv("ETHEREAL_PK"), restClient.Mainnet)
	if err != nil {
	...
	orders := make([]*restClient.Order, 3)
	for i := range orders {
		px := 1000.1 + float64(i)
	...
	cancel := restClient.NewCancelOrderFromCreated(placed...)
